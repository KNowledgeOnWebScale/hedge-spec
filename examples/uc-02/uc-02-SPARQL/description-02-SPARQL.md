...
Access by physician as data controller to specific subset of data (e.g. historical weight measurements recorded by HCP). Using a SPARQL query.

For that:
- The patient has a [policy](policy-02-SPARQL.ttl) allows a subset of her health data to be read for primary care.
- The physician issues a [request](request-02-SPARQL.ttl) to read a subset of health data for primary care with consent as legal basis.
